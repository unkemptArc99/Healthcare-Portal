    ...
    if(isset($_POST['generate']))
      {  
        ...
        if(isset($_COOKIE[$cookiename]))
        {
          ...
          $sql="insert into Appointment (PatientUsrName,StaffUsrName,Date) values('$q','$user','$date')";
          $result=mysqli_query($conn,$sql);
          if(!$result)
            echo mysqli_error($result); 
          $sql1="select * from Appointment where PatientUsrName='$q' AND StaffUsrName='$user' AND Date='$date' order by Date desc limit 1";
          ...
          while($row=mysqli_fetch_assoc($result1))
          {
            ...
            for($x=0;$x<$sel;$x++)
            {
              ...
              $qty=$qtyarray[$x];
              $dos=$dosarray[$x];
              $sql2="insert into Prescription (AppointmentID,MedicineID,Qty,Dosage,Date) values ('$app','$med','$qty','$dos','$date')";
