...
<body style="z-index:1">
  ...
  <?php
    ...
    {
      $username=$_COOKIE['usernamepat'];
      $sql="select * from Appointment where PatientUsrName='$username' order by Date desc";
      ...
      while($row=mysqli_fetch_assoc($result))
      {
        ...
        <td class="mdl-data-table__cell--non-numeric">'.$row['AppointmentID'].'</td>
        <td style="color:'.$color.' " class="mdl-data-table__cell--non-numeric">'.$status.'</td>  
      </tr>';
      }
    }
    mysqli_close($conn);
