...
<body style="z-index:1">
  ...
  <thead>
    <tr >
      <th class="mdl-data-table__cell--non-numeric">Appointment Id</th>
  ...
  <tbody>
    ...
    $username=$_COOKIE['usernamedoc'];  
    $sql="select * from Appointment where StaffUsrName='$username' and Status=0";
    $result=mysqli_query($conn,$sql);
    while($row=mysqli_fetch_assoc($result))
    {
    echo '
    <tr>
      <form action="pear/apptconf.php" method="POST">
      <td class="mdl-data-table__cell--non-numeric"><input style="font-style:italic; color:green" class="mdl-textfield__input" type="text" id="aid" name="aid" value="'.$row['AppointmentID'].'" /></td>
      <td class="mdl-data-table__cell--non-numeric">'.$row['Date'].'</td>
      <td class="mdl-data-table__cell--non-numeric">'.$row['Description'].'</td>
      <td class="mdl-data-table__cell--non-numeric" style="font-size:2px">
        <label class="mdl-radio mdl-js-radio mdl-js-ripple-effect" for="option1">
           <input type="radio" id="option1" name="radio" value="cnf" class="mdl-radio__button" >
           <span style="font-style:italic;color:green" class="mdl-radio__label">confirm</span>
        </label>

        <label class="mdl-radio mdl-js-radio mdl-js-ripple-effect" style="margin-left:20px" for="option2">
           <input type="radio" id="option2" name="radio" value="rj" class="mdl-radio__button" >
           <span style="font-style:italic;color:red" class="mdl-radio__label">Reject</span>
        </label>
